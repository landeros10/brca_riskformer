...
class RiskFormer_ViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.num_patches = 256  # Fixed for 256x256 patches
        ...
        if use_phi and phi_dim is not None:
            self.model_dim = input_embed_dim - phi_dim
        else:
            self.model_dim = input_embed_dim
        
        # Set up class tokens if needed
        ...
        # Initialize layers for local processing
        self.initialize_local_blocks()
        
        # Initialize layers for downscaling
        if self.downscale_depth > 0:
            self.initialize_downscale_blocks()
        
        # Initialize layers for global processing
        self.initialize_global_blocks()
        
        # Output head
        self.norm = nn.LayerNorm(self.output_embed_dim)
        self.global_pool = "token" if self.use_class_token else "avg"
        
        self.head = nn.Sequential(
            nn.Dropout(self.drop_rate),
            nn.Linear(self.output_embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        )
    ...
    def initialize_global_blocks(self):
        ...
        # Projection from model_dim to output_embed_dim
